...
function customAssign(...objects) {
  ...
  if (objects.length < 2)
    ...
    let resObj = Object.assign(...objects);
    for (const k in resObj) {
      if (typeof resObj[k] == "number") resObj[k] = resObj[k].toString();
    }
...
function customEntries(obj) {
  ...
  else return "Помилка: Аргумент не є об'єктом.";
}
...
function customObjectFromEntries(entries) {
  ...
  if (Array.isArray(entries)) {
    entries.map((value) =>
      typeof value[0] == "number" ? (value[1] = value[0].toString()) : value[0]
    );
    return Object.fromEntries(entries);
  } else return "Помилка: Вхідний аргумент має бути масивом.";
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  if (typeof obj1 == "object" && typeof obj2 == "object") {
    ...
    else return `об'єкт не було "заморожено"`;
  } else return null;
}
...
function checkAndPreventExtensions(obj) {
  ...
  if (typeof obj == "object") {
    if (Object.isExtensible(obj)) Object.preventExtensions(obj);
  ...
  if (Array.isArray(arr)) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (!resObj.hasOwnProperty(key))
        resObj = { ...resObj, ...Object.fromEntries([arr[i]]) };
      else {
        resObj[key] = value;
